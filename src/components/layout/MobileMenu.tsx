...
function MobileMenu({ onClose, isOpen }: MobileMenuProps) {
  ...
  return (
    <div
      className={`fixed left-0 top-0 z-[60] flex h-dvh w-full flex-col justify-between gap-8 overflow-y-auto hide-scrollbar border-t border-black bg-white shadow-md transition-opacity duration-300 ease-in-out ${isOpen ? "opacity-100" : "pointer-events-none opacity-0"
        }`}
    >
      <div
      ...
      <ul className="flex flex-col items-center gap-5 px-4 py-4 text-2xl font-semibold uppercase text-[#2E2E2E] sm:gap-7 sm:text-[32px]">
        {mobileNavLinks.map((link, index) => (
          <li
            ...
            className={`w-full transition-transform duration-300 ease-in-out ${isOpen ? "translate-y-0 opacity-100" : "-translate-y-8 opacity-0"
              }`}
      ...
      <div
        className={`flex flex-col items-center gap-4 px-4 pb-8 transition-transform duration-300 ease-in-out delay-500 sm:flex-row sm:items-end sm:justify-between sm:mt-16 ${isOpen ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
          }`}
      >
        <h2 className="text-5xl font-bold text-[#2E2E2E] sm:translate-y-5 sm:text-[66px]">
          IPOOL
        </h2>
        <div className="flex flex-col items-start justify-end text-[17px] font-normal sm:items-end">
          <a href="tel:78632838281">+7 863 283-82-81</a>
          <a href="tel:79185538734">+7 918 553-87-34</a>
          <a href="mailto:info@ipool.ru" className="mb-16 text-[17px] uppercase">
            info@ipool.ru
          </a>
        </div>
      </div>
